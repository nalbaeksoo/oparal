...
During execution a progress line appears every 10 seconds showing completed percentage, the number of running slave processes and how many have been forked in total. CPU and memory statistics are extracted from `top -bn1` and CPU usage is displayed to one decimal place (values under `0.05%` round up to `0.1%`). When finished, a results file named `YYYYMMDD_HHMM_INSTANCE.result.csv` is written containing start time, end time and duration for each file. The instance identifier keeps results from concurrent runs separate.
